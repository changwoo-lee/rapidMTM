#' SBM with multiple-try metropolis with max(1,u) weight function
...
#'
#' @examples
sbm_mtm_max<- function(A, k =5, ntry = 10, z_init = NULL, N_iter=10000, a=1, b=1, 
  ...
  for (imcmc in 1:N_iter){
    ...
    logweights = pmax(log_lik_new - log_lik, 0)
    ...
    logweights_star = pmax(log_lik_newstar - proposed.loglik, 0)
    
    accratio.denom = matrixStats::logSumExp(c(logweights_star, pmax(log_lik - proposed.loglik, 0) )) # log scale
