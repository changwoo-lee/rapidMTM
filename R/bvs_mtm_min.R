#' BVS with multiple-try metropolis with min(1,u) weight function
...
#' @param K the number of multiple-trials
...
#'
#' @examples
bvs_mtm_min <- function(y, X, g, kappa, s0, burn = 1000, nmc = 5000, thin = 1,
                             gammainit = NULL, 
  ...
  for(imcmc in 1:nmcmc){
    ...
    for(jj in Kidx_drop){
      ...
      logweights[jj] = pmin(logpost_multiple[jj] - logpost.current, 0)
    }
    ...
    if( s+1 > s0){#do nothing if snew > s0
      ...
      logweights[Kidx_add] = pmin(logpost_multiple[Kidx_add] - logpost.current, 0)
    }
    ...
    idx_K_star = sample.int(p, size = K-1, replace = T) # auxiliary 
    ...
    logpost_multiple_star = numeric(K-1) # length K -  save log posterior probabilities
    ...
    for(jj in Kidx_drop_star){
      ...
      logweights_star[jj] = pmin(logpost_multiple_star[jj] - logpost.proposed, 0)
    }
    ...
    if( snew+1 > s0){#do nothing if snew > s0
      ...
      #if(any(is.na(K_weight_normalized))) browser()
      logweights_star[Kidx_add_star] = pmin(logpost_multiple_star[Kidx_add_star] - logpost.proposed, 0)
    }
    #########################################################################################
    if(any(is.na(logpost_multiple_star))) browser()
    
    accratio.denom = matrixStats::logSumExp(c(logweights_star, pmin(logpost.current - logpost.proposed, 0) )) # log scale
