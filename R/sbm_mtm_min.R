#' SBM with multiple-try metropolis with min(1,u) weight function
...
#'
#' @examples
sbm_mtm_min<- function(A, k =5, ntry = 10, z_init = NULL, N_iter=10000, a=1, b=1, 
  ...
  for (imcmc in 1:N_iter){
    ...
    logweights = pmin(log_lik_new - log_lik, 0)
    ...
    logweights_star = pmin(log_lik_newstar - proposed.loglik, 0)
    
    accratio.denom = matrixStats::logSumExp(c(logweights_star, pmin(log_lik - proposed.loglik, 0) )) # log scale
