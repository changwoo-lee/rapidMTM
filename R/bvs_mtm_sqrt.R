...
#'
#' @examples
bvs_mtm_sqrt <- function(y, X, g, kappa, s0, burn = 1000, nmc = 5000, thin = 1,
                             gammainit = NULL, 
  ...
  for(imcmc in 1:nmcmc){
    ...
    for(jj in Kidx_drop){
      ...
      logweights[jj] = (logpost_multiple[jj] - logpost.current)/2
    }
    ...
    if( s+1 > s0){#do nothing if snew > s0
      ...
      logweights[Kidx_add] = (logpost_multiple[Kidx_add] - logpost.current)/2
    }
    ...
    for(jj in Kidx_drop_star){
      ...
      logweights_star[jj] = (logpost_multiple_star[jj] - logpost.proposed)/2
    }
    ...
    if( snew+1 > s0){#do nothing if snew > s0
      ...
      #if(any(is.na(K_weight_normalized))) browser()
      logweights_star[Kidx_add_star] = (logpost_multiple_star[Kidx_add_star] - logpost.proposed)/2
    }
    #########################################################################################
