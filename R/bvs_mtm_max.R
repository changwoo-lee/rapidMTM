#' BVS with multiple-try metropolis with max(1,u) weight function
...
#'
#' @examples
bvs_mtm_max <- function(y, X, g, kappa, s0, burn = 1000, nmc = 5000, thin = 1,
                             gammainit = NULL, 
  ...
  for(imcmc in 1:nmcmc){
    ...
    for(jj in Kidx_drop){
      ...
      logweights[jj] = pmax(logpost_multiple[jj] - logpost.current, 0)
    }
    ...
    if( s+1 > s0){#do nothing if snew > s0
      ...
      logweights[Kidx_add] = pmax(logpost_multiple[Kidx_add] - logpost.current, 0)
    }
    ...
    for(jj in Kidx_drop_star){
      ...
      logweights_star[jj] = pmax(logpost_multiple_star[jj] - logpost.proposed, 0)
    }
    ...
    if( snew+1 > s0){#do nothing if snew > s0
      ...
      #if(any(is.na(K_weight_normalized))) browser()
      logweights_star[Kidx_add_star] = pmax(logpost_multiple_star[Kidx_add_star] - logpost.proposed, 0)
    }
    #########################################################################################
    if(any(is.na(logpost_multiple_star))) browser()
    
    accratio.denom = matrixStats::logSumExp(c(logweights_star, pmax(logpost.current - logpost.proposed, 0) )) # log scale
